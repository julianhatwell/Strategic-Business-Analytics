...
# plot sales volume (sum) against sd of sales

# high sales, low var: horses are strong and reliable
# high sales, high var: wild bulls, strong and unpredictable
# high sales, high var: crickets, small and can jump unexpectedly

# where var is low, forecast error is low. horses can be made to keep in stock
# crickets can be made to order because variability is high and returns are low
# bulls, case by case. 
...
xyplot(ADS~ CV
       , main = "After Clustering"
       , type="p", group=groups
       , data=data
       , auto.key=list(title="Group"
                       , cex=1.0
                       , just = 0.95
                       , col=myCols
                       , points = FALSE
...
testdata <- data # To keep our dataset safe, let's create a copy of it called "testdata"
# testdata <- data[, 1:3] # for the quiz
