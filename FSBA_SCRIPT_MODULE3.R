...
dat <- read.table('DATA_3.01_CREDIT.csv',sep=',',header=TRUE) # The function read.table enables us to read flat files such as .csv files
...
hist(dat$Rating) # Produce a histogram of the credit scores
cor(dat[,c(1:5,10)]) # Compute the correlation between all the numerical variables of the sample

# create a linear model
linreg <- lm(Rating~.,data=dat) # Estimate a linear regression model of Rating as a function of everything else.

# in sample accuracy (not tested on unseen data)
cor(linreg$fitted.values,dat$Rating) # Computes the correlation between the fitted values and the actual ones
plot(dat$Rating,linreg$fitted.values) # Plot the fitted values vs. the actual ones

summary(linreg) # Reports the results of the regression

plot(dat$Balance,dat$Rating) # Allows to visualize the relationship between Balance and Rating
plot(dat$Income,dat$Rating) # Allows to visualize the relationship between Income and Rating

# the resid plot is totally non-linear
plot(dat$Rating, rstandard(linreg))
plot(linreg)

linreg.poly2 <- lm(Rating~poly(Balance, 2) + Income + Student, data = dat)
# same
linreg.quad <- lm(Rating~I(Balance^2) + Balance + Income + Student, data = dat)

cor(linreg.quad$fitted.values,dat$Rating) # Computes the correlation between the fitted values and the actual ones
plot(dat$Rating,linreg.quad$fitted.values) # Plot the fitted values vs. the actual ones

plot(linreg.quad)
plot(dat$Rating, rstandard(linreg.quad)) # it's not that much better


# dealing with the skew here
hist(dat$Rating) # Produce a histogram of the credit scores

library(forecast) # to find optimal lambda
lambda <- -BoxCox.lambda(dat$Rating)

# alternative
bc <- boxcox(Rating~1, data = dat)
lambda.alt <- bc$x[order(-bc$y)][1]

# 0.25 is good enough
linreg.bc <- update(linreg.quad, I(Rating^0.25)~.)
plot(dat$Rating, rstandard(linreg.bc))

# that's better but nowhere near good enough
...
table(logreg$fitted.values > cutoff, datatot$left)/nrow(datatot)
